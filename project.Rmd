...
# Hypothesis
## Hypo 1
Without capitation or fee-for-service incentives and with low monitoring, providers may induce less services. 
```{r}
# Label providers
health_quality_data <- health_quality_data %>%
  mutate(
...
## Hypo 2: Low effort per treatment
Without capitation or fee-for-service incentives and with low monitoring, providers put less effort per patient.
```{r}
health_quality_data <- health_quality_data %>%
...
## Hypo 3: Moral hazard leads to inefficient care, not better care.
```{r}
## ----------------- HYPOTHESIS 3 --------------------
...
Does more test or more medication significantly improve health outcome?
...
health_quality_data <- health_quality_data %>%
  mutate(
    health_outcome = ifelse(P7_2 %in% c(1), 1, 0)
  )
...
# 5) Draw them
plot_by_system(test_df, "Effect of Test Requested on Return Propensity, by Provider")
plot_by_system(meds_df, "Effect of Number of Medications on Return Propensity, by Provider")

```

Testing and patient out-of-pocket spending
```{r}
...
health_quality_data$provider <- relevel(health_quality_data$provider, ref = "Private insurance")

provider_model <- lm(
  health_outcome ~ provider * effort_index + age + sex + primary_condition,
  data = health_quality_data
)

provider_df <- tidy(provider_model, conf.int = TRUE) %>%
  filter(grepl("^provider", term)) %>%
  # drop the reference level (intercept) row if it sneaks in
  filter(term != "(Intercept)") %>%
  mutate(
...
plot_by_system <- function(df, title_text) {
  ggplot(df, aes(x = fct_reorder(system, original_order), y = estimate, color = estimate > 0)) +
    geom_pointrange(aes(ymin = conf.low, ymax = conf.high), fatten = 1) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
    scale_color_manual(values = c("firebrick", "steelblue")) +
    geom_text(aes(label = paste0(
    ...
    labs(title = title_text, y = "Coefficient (95% CI)", x = "") +
    theme_minimal(base_size = 13) +
    theme(legend.position = "none") +
    coord_flip()
}

plot_by_system(provider_df, "Effect of Provider on Health Outcome")
```

Is our effort index robust? - Test whether effort index is positively and significantly contributing to health outcomes. 
```{r}

effort_df <- health_quality_data %>%
  group_by(provider, system) %>%
  do(tidy(
    lm(health_outcome ~ effort_index + age + sex + factor(primary_condition), data = .),
    conf.int = TRUE
  )) %>%
  filter(term == "effort_index") %>%
  mutate(original_order = row_number())
...
plot_by_system(effort_df, "Effect of Effort on Health Outcome, by System")
```
